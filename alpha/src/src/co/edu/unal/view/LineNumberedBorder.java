...
public class LineNumberedBorder extends AbstractBorder {
	...
	private int lineNumberWidth(JTextArea textArea) {
		...
		//
		int lineCount = Math.max(textArea.getRows(),
	...
	public void paintBorder(Component c, Graphics g, int x, int y, int width,
		...
		Color parentColor=c.getForeground();
		g.setColor(parentColor);
		//
		// loop until out of the "visible" region...
		//
		int length = ("" + Math.max(jta.getRows(), jta.getLineCount() + 1))
				.length();
		while (ybaseline < yend) {
			...
			//
			if (errorLines.contains(startingLineNumber-1)){
			...
			if (lineNumberJustification == LEFT_JUSTIFY) {
				...
				g
						.drawString(label, lnxstart - fm.stringWidth(label),
								ybaseline);
			}
